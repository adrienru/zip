...
async function selectDirs() {
    const lArg = yargs.getArgv('l');
    const rArg = yargs.getArgv('r') || '/BotTelegram';

    if (!lArg || !fs.existsSync(path.resolve(lArg))) {
        console.error('[ERROR] Invalid or missing local path (-l)');
        process.exit(1);
    }
    if (!rArg) {
        console.error('[ERROR] Missing remote path (-r)');
        process.exit(1);
    }

    const localDir = path.resolve(lArg);
    const remoteDir = rArg;
